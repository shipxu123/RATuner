...
class Morbo: 
    ...
    def __init__(self, nEmbs, nDims, nObjs, nameVars, typeVars, rangeVars, funcEval, refpoint, weights, scale=1.0, nInit=2**4, batchSize=2, mcSamples=1024, nJobs=4): 
        ...
        # self._A = self._getA()
        self._A = self._get_TFIDF_A()

        self._visited = {}

    def _get_TFIDF_A(self): 
        matLast = np.random.randn(self._nDims, self._nEmbs)
        with open('/home/pxu/codes/RATuner/results/tfidf-embeddings/command_embedding.json', 'r') as file:
            embeddings_data = json.load(file)

        print(embeddings_data.keys())
        print(self._name2index.keys())

        embeddings = np.array([v for v in embeddings_data.values()]).squeeze(1)
        pdb.set_trace()
        kmeans = MiniBatchKMeans(n_clusters=self._nEmbs, init_size=1024, batch_size=2048, random_state=20).fit(embeddings)
        assignment_matrix = np.zeros((self._nDims, self._nEmbs), dtype=int)
        assignment_matrix[np.arange(embeddings.shape[0]), kmeans.labels_] = 1

        return matLast * assignment_matrix
...
if __name__ == "__main__": 
    ...
    def tmpFuncEval(config, index=0):

        a, b, c = random.random() * 50, random.random() * 50, random.random() * 50
    ...
    ndims = len(names)
    model = Morbo(nembs, ndims, nobjs, names, types, ranges, tmpFuncEval, refpoint=[refpoint, ] * nobjs, weights=[1.0/nobjs] * nobjs, scale=float(args.scale), nInit=ninit, batchSize=int(args.batchsize), nJobs=int(args.njobs))
